...
import config
import pandas as pd
from signals import check_all_signals, check_scores, save_signals
from sqlalchemy import create_engine
from trading_monitor import add_tradingview_metrics, get_available_tickers, get_candles
...
def main(
    ...
    while True:
        ...
        """
        now = pd.Timestamp = pd.to_datetime(datetime.now().astimezone()).tz_convert(
            "Europe/London"
        )
        ...
        candles = get_candles(
            stock_id=stock_id, start=start, end=end, interval=interval, engine=engine
        )
