...
def get_candles(
    stock_id: int = None,
    start=pd.to_datetime(datetime.now().astimezone()).tz_convert("Europe/London")
    - pd.Timedelta(days=7),
    end: pd.Timestamp = pd.to_datetime(datetime.now().astimezone()).tz_convert(
        "Europe/London"
    ),
    interval: str = "1m",
    ...
    try:
        if df.empty:
            ...
            ichimoku = ta.trend.IchimokuIndicator(
                df["high"], df["low"], window1=9, window2=26, window3=52, visual=False
            )
...
def add_oscilators(df: pd.DataFrame = None) -> pd.DataFrame:
    try:
        if df.empty:
            ...
            df["aws_os"] = ta.momentum.AwesomeOscillatorIndicator(
            ...
            df["ultimate_os"] = ta.momentum.UltimateOscillator(
                high=df["high"],
                low=df["low"],
                close=df["close"],
                window1=7,
...
def add_tradingview_metrics(
    df: pd.DataFrame = None, tail: Optional[bool] = False
) -> pd.DataFrame:
    try:
        if df.empty:
            print("No df provided")
